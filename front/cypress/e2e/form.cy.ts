...
describe('Session spec', () => {
    ...
    it('user not admin redirect to sessions', () => {
        cy.visit('/login')
    ...
    it('update session, user admin', () => {
        ...
        cy.intercept('PUT', '/api/session/1', {
            body: {
                name: "Meditation Session",
                description: "relaxing meditation session BEGINNER",
                date: "2025-03-02T00:00:00Z",
                teacher_id: 1,
                users: [1, 2, 3],
                createdAt: "2025-01-01T00:00:00Z",
        ...
        cy.get('textarea[formControlName=description]').type("relaxing meditation session BEGINNER")
        cy.get('button').contains('Save').click()
        ...
        ]).as('sessionAfterUpdateSession')

        cy.wait('@updateSession');
        cy.wait('@sessionAfterUpdateSession')

        cy.url().should('include', '/session')

    });

    it('update session failed, user admin', () => {
        cy.visit('/login')
